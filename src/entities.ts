...
let entities:entities_type = {
    ...
    background: {
        ...
        update: (d, o, t, dt) => {
            ...
            // Clouds
            let clouds:number = 20;
            let cw:number = 16*3;
            for (let n:number = 1; n <= clouds; n++) {
                let r:number = algo.rand(n+d.data);
                let p:number[] = [algo.rand(r+1), algo.rand(r+2)];
                o.sprites('Cloudsv1 (1).png', [algo.mod((o.w+2*cw)*p[0]-o.camera[0]*Math.pow(n/clouds,5)*0.5+cw, o.w+cw)-cw, o.h*Math.pow(p[1],2)],
