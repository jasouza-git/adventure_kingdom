...
let lv = level(main);
let bg = main.entity('background', {house:true, data: Number(new Date())});
...
main.scene('into', (t,dt) => {
    v.play();
    main.on('Enter,gp_1,gp_2,gp_3,gp_4', load_level);
});
...
main.scene('level', (t, dt) => {
    ...
    if (player.canclimb && main.on('w,W,s,S,gp_n,gp_s')) {player.climb = 1; player.m = [0,0]}
